...
if dtype_routine == "adm":
    for origin_file in data_files:
        ...
        df_dic[file], variable_types, data_types = d_type.run(df_dic[file], method=dtype_routine)
        df_meta = pd.merge(variable_types, data_types, on=["column"])  # todo check!
        df_meta["tablename"] = "%s/%s" % (dtype_routine, file)  # todo check!
...
for file in data_files:
    ...
    if sv_test:
        if test_mode:
            ...
            train_file = file
        df_dic[file] = redshif.read(table_name=train_file, routine_name=dtype_routine)  # todo for REAL !
...
if sv_test:
    original_df = redshif.read(table_name="merge", routine_name="adm")  # todo for REAL!
